...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgres://Bradley_Scharf@localhost/scharfbHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
search_gifs = db.Table('search_gifs',
    db.Column('search_id', db.Integer, db.ForeignKey("search_term.id"), primary_key=True),
...
user_collection = db.Table('user_collection',
    db.Column("gif_id", db.Integer, db.ForeignKey("gifs.id"), primary_key=True),
    db.Column("collection_id", db.Integer, db.ForeignKey("personal_collection.id"), primary_key=True)
)
...
class User(UserMixin, db.Model):
    ...
    collections = db.relationship('PersonalGifCollection', backref='owner')
...
class Gif(db.Model):
    ...
    def __repr__(self):
        return f"{self.title} : {self.embedURL}"
...
class PersonalGifCollection(db.Model):
    ...
    #owner = db.relationship("User", backref=db.backref("collections", uselist=True))
    ...
    gifs = db.relationship('Gif', secondary=user_collection, lazy="dynamic",
...
class SearchTerm(db.Model):
    ...
    #gifs = db.relationship(Gif, secondary="search_gifs")
    gifs = db.relationship('Gif', secondary=search_gifs, lazy='dynamic',
...
def get_or_create_search_term(term):
    ...
    # It should iterate over that list acquired from Giphy and invoke get_or_create_gif for each, and then append the return value from get_or_create_gif to the search term's associated gifs (remember, many-to-many relationship between search terms and gifs, allowing you to do this!).
        for g in gifs:
            check_gif = get_or_create_gif(g['title'], g['embed_url'])
            add_gifs.add(check_gif)
    ...
    # And the SearchTerm instance that was got or created should be returned.
        search = SearchTerm(term=term, gifs=list(add_gifs))
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    #collection = PersonalGifCollection.query.filter_by(name=name,owner_id=current_user).first()
    collection = PersonalGifCollection.query.filter_by(name=name,owner=current_user).first()
    ...
    if not collection:
        collection = PersonalGifCollection(name=name, owner=current_user, gifs=gif_list)
...
def create_collection():
    ...
    form.gif_picks.coerce = int
    if form.validate_on_submit():
        get_gifs = [get_gif_by_id(id) for id in form.gif_picks.data]
        collection = get_or_create_collection(form.name.data, current_user, get_gifs)
...
def collections():
    ...
    collections = PersonalGifCollection.query.filter_by(owner=current_user).all()
